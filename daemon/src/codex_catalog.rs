...
/// Where the Codex build the catalog was generated from is recorded.
///
/// The entry schema is unversioned, and Codex refuses to start *at all* when
/// `model_catalog_json` names a document it cannot deserialize — one added
/// field upstream and every Codex command dies on
/// `missing field ... at line N`, the model picker and the `debug models` dump
/// we would rebuild from included. Stamped in a file of our own rather than
/// inside the catalog, whose shape is Codex's to define.
fn version_path() -> PathBuf {
    home().join(".asale").join("codex-version")
}

/// The installed Codex builds, as they identify themselves. `--version` reads
/// no config, so it still answers when the catalog is the broken thing.
fn installed_version() -> String {
    codex_binaries()
        .iter()
        .filter_map(|bin| Command::new(bin).arg("--version").output().ok())
...
/// Was the catalog on disk generated by the Codex that is installed now?
///
/// `false` means Codex is about to reject it — or already is — and the config
/// has to be re-applied before it will run again.
pub fn matches_installed_codex() -> bool {
    if !path().exists() {
        return true;
    }
    std::fs::read_to_string(version_path()).unwrap_or_default() == installed_version()
}
...
pub fn write(models: &[String]) -> Result<Option<Generated>> {
    ...
    // Stamped last: a catalog with no stamp beside it reads as stale, which
    // costs one rebuild rather than leaving Codex unable to start.
    write_atomic(&version_path(), &installed_version())?;
