...
```bash
pnpm install
# 起 daemon + Tauri 視窗（注入 ASALE_QUOTA_PUBKEY）
pnpm dev:app
# workspace 全量測試
cargo test
...
```bash
# debug 建置，軟連結到 /usr/local/bin（要 sudo）
./scripts/link.sh
# 啟動快、體積小，編譯慢
./scripts/link.sh --release
# 不動 /usr/local/bin，也就不用 sudo
./scripts/link.sh --prefix ~/.local/bin
# 現在的 asale / asaled 指向哪裡
...
```bash
# 填 ASALE_QUOTA_PUBKEY，缺了打出來的用戶端不能賣出
cp .env.package.example .env.package

# macOS → .dmg（預設 arm64 + x86_64 通用二進位檔）
./scripts/package.sh
# 在 Linux 上
...
更新就是重新跑一遍官方安裝指令稿。用戶端向 `https://asale.ai/dl/manifest.json` 問目前發佈版本
（背景每十分鐘一次，設定頁也可手動觸發），跟自己的版本比對，要更新時執行的正是官網給出的那條指令
—— `curl -fsSL https://asale.ai/dl/install.sh | sh`，Windows 是對應的 PowerShell 版本。

沒有增量式自動更新，這是刻意的：桌面應用程式和 `asale` / `asaled` 命令列屬於同一個發佈版本，
卻是分別落到機器上的，只有那個安裝指令稿能把兩邊一起換掉；自更新器只能修好視窗，會讓終端機裡
悄悄停在上一個版本。代價是要輸一次管理者密碼（命令列裝在 root 所有的目錄裡），所以流程是
應用程式關閉 → 提權安裝 → 自動重新開啟（見 `src-tauri/src/updater.rs`）。
