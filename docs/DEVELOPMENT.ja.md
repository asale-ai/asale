...
更新は公開されているインストーラーを実行し直すことで行います。クライアントは
`https://asale.ai/dl/manifest.json` に現在のリリースを問い合わせ（バックグラウンドで
10 分ごと、設定画面から手動でも）、自身のバージョンと比較して、更新時にはサイトが案内している
コマンドそのものを実行します —— `curl -fsSL https://asale.ai/dl/install.sh | sh`、
Windows は対応する PowerShell 版です。

署名済み差分の自動更新機構は、意図的に持ちません。デスクトップアプリと `asale` / `asaled`
コマンドラインは同じリリースでありながら別々にマシンへ届き、両方を置き換えられるのはその
インストーラーだけです。自己更新機構ではウィンドウしか直せず、ターミナル側が黙って古いまま
残ります。代償は管理者パスワードの入力です（コマンドラインは root 所有のディレクトリにある
ため）。したがってアプリを終了 → 権限昇格してインストール → 自動で再起動、という流れになります
（`src-tauri/src/updater.rs` を参照）。
