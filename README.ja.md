...
<sup>`v0.1.0-6` 以前のビルドは署名を設定する前に作られたもので、macOS が「Apple は悪意のある
ソフトウェアが含まれていないことを確認できません」としてブロックします。新しいリリースに更新して
ください。それでも古いビルドを開く場合は**システム設定 → プライバシーとセキュリティ**を開き、
一番下までスクロールして「このまま開く」をクリックします —— macOS 15 で Control クリック → 開く
の回避策は廃止され、これが唯一の方法です。</sup>
